...
export async function POST(req: Request) {
  try {
    ...
    const timestampedOutline =
      typeof body.timestampedOutline === 'string'
        ? body.timestampedOutline.trim()
        : '';
    ...
    const result = await runShowrunnerEpisodeMetadata({
      transcript,
      episodeType,
      guestName,
      guestLinks,
    ...
    return NextResponse.json({
      ...
      youtubeTitle: result.parsed.youtubeTitle,
      spotifyTitle: result.parsed.spotifyTitle,
      tokensIn: result.result.inputTokens,
      tokensOut: result.result.outputTokens,
      costEstimate: result.result.costEstimate,
    });
  } catch (e) {
    const msg = e instanceof Error ? e.message : 'Metadata run failed';
